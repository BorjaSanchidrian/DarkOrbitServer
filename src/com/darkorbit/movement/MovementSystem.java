	...
	public void run() {
		...
		while(currentThread == thread) {
	        ...
	        for(Entry<Integer, Portal> p : GameManager.portals.entrySet()) {
	        	if(p.getValue().getMapID() == player.getMapID()) {
	        		if(player.isInRange(p.getValue()) && p.getValue().isSafe() && (Calendar.getInstance().getTimeInMillis() - player.getLastAttack()) >= 2000) {
		        		//0|D|playerX|playerY|zonaNoAtaques(bool)|repBot(bool)|trade(bool)|radiacion(bool)|0 (IDK)|instantRepair(int)
		        		sendPacket(userSocket, "0|D|" + player.getPosition().getX() + "|" + player.getPosition().getY() + "|1|0|0|0|0|0");
	        ...
	        if(!safeZone) {
	        	//0|D|playerX|playerY|zonaNoAtaques(bool)|repBot(bool)|trade(bool)|radiacion(bool)|0 (IDK)|instantRepair(int)
        		sendPacket(userSocket, "0|D|" + player.getPosition().getX() + "|" + player.getPosition().getY() + "|0|0|0|0|0|0");
        		player.isInSafeZone(false);
	        }

	        //Check is the player continue moving
	        playerMoving();
		}
	}
